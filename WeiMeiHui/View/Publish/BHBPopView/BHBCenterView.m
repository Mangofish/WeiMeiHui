...
- (void)layoutBtnsWith:(NSArray *)items isMore:(BOOL)isMore{
    ...
    for (int i = 0; i < items.count; i ++) {
        ...
        [btn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@",item.icon]] forState:UIControlStateNormal];
...
//        CGFloat x = (i % 3) * self.frame.size.width / 3.0;
        ...
        CGFloat x =  self.frame.size.width*215/750 + (i)*(self.frame.size.width*80/750 +60);
...
//        CGFloat width = self.frame.size.width*120/750;
        ...
        [self addSubview:btn];
        btn.frame = CGRectMake(x, y, 60, height);
    }
}
