...
@property (nonatomic, strong) UIButton *currentCityBtn;
...
//    searchBar.layer.masksToBounds = YES;
    ...
    searchBar.placeholder = @"输入城市名/拼音";
    [self addSubview:searchBar];
    self.searchBar = searchBar;
    
    [searchBar mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.offset(self.bounds.size.width - 40);
...
- (void)addLabels {
    ...
    _currentCityLabel.textColor  = JFRGBAColor(51, 51, 51, 1.0);
    ...
    [_currentCityLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.offset(kWidth-Space*4);
        make.height.offset(44);
        make.left.equalTo(currentLabel.mas_right).offset(5);
        make.bottom.equalTo(self.mas_bottom).offset(0);
    }];
    
    self.currentCityBtn = [[UIButton alloc] init];
    [_currentCityBtn addTarget:self action:@selector(chooseLocate:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_currentCityBtn];
...
- (void)chooseLocate:(UIButton *)sender{
...
- (void)addJFButton {
    ...
    self.button = [[UIButton alloc] initWithFrame:CGRectMake(0, SafeAreaTopHeight, 40, 40)];
    [_button addTarget:self action:@selector(touchUpJFButtonEnevt:) forControlEvents:UIControlEventTouchUpInside];
    [_button setImage:[UIImage imageNamed:@"返回新"] forState:UIControlStateNormal];
