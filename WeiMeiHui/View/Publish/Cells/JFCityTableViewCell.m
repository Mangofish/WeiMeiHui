...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, Space, kWidth-30, self.bounds.size.height) collectionViewLayout:[[JFCityCollectionFlowLayout alloc] init]];
...
//    NSIndexSet *path = [NSIndexSet indexSetWithIndex:0];
//    [_collectionView reloadSections:path];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.title = _cityNameArray[indexPath.item];
    return cell;
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    NSString *cityName = _cityNameArray[indexPath.item];
    NSLog( @"cityNameArraySelect：%@",_cityNameArray);
