...
-(UIScrollView *)scrollView{
    if (!_scrollView) {
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth-80, ScreenWidth-80)];
...
-(void)creatItemView{
    ...
    for ( int i = 0; i < _itemsCount; i++ ) {
        ...
        DYItemView*item = [[DYItemView alloc]initWithFrame:CGRectMake((ScreenWidth-80)/2-200/2+i*ScrollWidth,40, 200, 200)];
        item.userInteractionEnabled = YES;
        item.index  = i;
        item.tag    = BaseTag+item.index;
        [item.imageView sd_setImageWithURL:[NSURL urlWithNoBlankDataString:adModel.imgStr] placeholderImage:[UIImage imageNamed:@"test2"]];
...
-(void)pageValueChange:(UIPageControl*)page{
    ...
    [UIView animateWithDuration:.35 animations:^{
        self->_scrollView.contentOffset = CGPointMake(page.currentPage*ScreenWidth, 0);
    }];
}
...
- (UILabel *)detailLab{
    ...
    if (!_detailLab) {
        _detailLab = [[UILabel alloc]initWithFrame:CGRectMake(0,265,ScreenWidth-80, 20)];
        _detailLab.backgroundColor        = [UIColor colorWithWhite:1 alpha:1.0];
        _detailLab.text = @"请联系店内手艺人进行扫码";
...
-(UIImageView *)imageView{
    if (!_imageView) {
        ...
        _imageView.backgroundColor        = [UIColor colorWithWhite:1 alpha:1.0];
