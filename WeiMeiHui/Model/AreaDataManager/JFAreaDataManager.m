...
- (void)searchCityData:(NSString *)searchObject result:(void (^)(NSArray *result))result {
    
    NSMutableArray *resultArray = [[NSMutableArray alloc] init];
    
    
    if (![[NSUserDefaults standardUserDefaults] valueForKey:WEICITYALL]) {
        
        FMResultSet *areaResult = [self.db executeQuery:@"SELECT city_name,city_number FROM shop_area"];
        
        while ([areaResult next]) {
            //            NSString *area = [areaResult stringForColumn:@"area_name"];
            NSString *city = [areaResult stringForColumn:@"city_name"];
            NSString *cityNumber = [areaResult stringForColumn:@"city_number"];
            NSDictionary *dataDic = @{@"super":city,@"city_number":cityNumber};
            
            if ([resultArray containsObject:dataDic]) {
                continue;
            }
            
            [resultArray addObject:dataDic];
            
            [[NSUserDefaults standardUserDefaults] setValue:resultArray forKey:WEICITYALL];
            [[NSUserDefaults standardUserDefaults] synchronize];
            
        }
    }else{
        resultArray = [[NSUserDefaults standardUserDefaults] valueForKey:WEICITYALL];
    }
    ...
    [ZYPinYinSearch searchByPropertyName:@"super" withOriginalArray:resultArray searchText:searchObject success:^(NSArray *results) {
        
        if (results.count == 0) {
                results = @[@{@"city_number":@"抱歉",@"super":@"未找到相关位置，可尝试修改后重试!"}];
            }
