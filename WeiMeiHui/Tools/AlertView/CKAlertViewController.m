...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        if ([self.actions[i].title isEqualToString:@"确定"]) {
...
- (void)updateAllButtonsFrame {
    ...
    CGFloat buttonW = self.actions.count>2 ? _contentViewWidth : _contentViewWidth/self.actions.count;
    
    for (int i=0; i<self.actions.count; i++) {
        UIButton *btn = [_contentView viewWithTag:10+i];
        CGFloat buttonX = self.actions.count>2 ? 0 : buttonW*i;
        CGFloat buttonY = self.actions.count>2 ? firstButtonY+_buttonHeight*i : firstButtonY;
        
        btn.frame = CGRectMake(buttonX, buttonY, buttonW, _buttonHeight);
    }
}
