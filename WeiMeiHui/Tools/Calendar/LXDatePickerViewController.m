...
- (void)onConfirmButtonClick {
    ...
    NSString *dateString = [NSString stringWithFormat:@"%@.%@.%@",self.yearString, self.monthString, self.dayString];
...
- (UIButton *)cancelButton {
    ...
    if (!_cancelButton) {
        ...
        [_cancelButton setTitleColor:LightFontColor forState:UIControlStateNormal];
...
- (UIButton *)confirmButton {
    ...
    if (!_confirmButton) {
        ...
        [_confirmButton setTitleColor:FontColor forState:UIControlStateNormal];
...
- (NSMutableArray *)yearArray {
    ...
    NSUInteger count  = [NSDate date].getYear;
    if (!_yearArray) {
        _yearArray = [NSMutableArray array];
        for (NSUInteger i = 1950; i <= count ; i++) {
            NSString *year = [NSString stringWithFormat:@"%04lu", (unsigned long)i];
