...
- (instancetype)initWithView:(UIView *)view items:(NSArray *)itemArr{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        ...
        _limitMargin = 40.0f;
        _dismissed = YES;
        CGRect convertFrame = [view convertRect:view.frame toView:self];
...
//        CGFloat tableViewX = viewCenterX - _menuWidth * 0.5 + _menuWidth * 0.5;
        CGFloat tableViewX = kWidth - _menuWidth - margin;
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(kWidth-Space-_menuWidth, SafeAreaHeight, _menuWidth, orginTableViewH) style:UITableViewStylePlain];
...
//        _tableView.layer.anchorPoint = tableViewX > [UIScreen mainScreen].bounds.size.width * 0.5 ? CGPointMake(1.0f, 0.0f) :CGPointMake(0.0f, 0.0f);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.separatorInset = UIEdgeInsetsMake(40, Space, 0, Space);
    return cell;
}
