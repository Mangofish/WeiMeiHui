...
- (void)configBlankPage:(EaseBlankPageType)blankPageType hasTitles:(NSString *)title hasImageNames:(NSString *)imgName hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
    if (hasData) {
        ...
        [self.blankPageView configWithType:blankPageType hasTitle:title hasImageName:imgName hasData:hasData hasError:hasError reloadButtonBlock:block];
    }
}
