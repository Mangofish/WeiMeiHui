...
- (void)config {
	...
	UIImageView *topLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, -1, kWidth, 1)];
...
- (void)setTabBarItemAttributes:(NSArray<NSDictionary *> *)tabBarItemAttributes {
    ...
    CGFloat normalItemWidth = (kWidth * 3 / 4) / (_tabBarItemAttributes.count - 1);
    CGFloat tabBarHeight = CGRectGetHeight(self.frame);
    CGFloat publishItemWidth = (kWidth / 4);
...
//            CGRect frame = CGRectMake(itemTag * normalItemWidth + (passedRiseItem ? publishItemWidth : 0), 0, type == LLTabBarItemRise ? publishItemWidth : normalItemWidth, type == LLTabBarItemRise ? 63 : tabBarHeight );
