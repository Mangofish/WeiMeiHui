...
#import "AuthorResultsViewController.h"
#import "ALLAuthorsTableViewCell.h"
#import "JSDropDownMenu.h"
#import "AuthorDetailViewController.h"

@interface AuthorResultsViewController ()<JSDropDownMenuDelegate,JSDropDownMenuDataSource>{
...
@implementation AuthorResultsViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.author = [ShopAuthor shopAuthorWithDict:self.authordataAry[indexPath.section]];
    return cell;
...
- (void)getDataAtIndex:(NSUInteger)tag{
    ...
    NSString *url = [PublicMethods dataTojsonString:@{@"uuid":uuid,@"city_id":city_id,@"type":@"5",@"lat":lat,@"lng":lng,@"page":@(self.page),@"dump_id":self.dump_id,@"intelligent":self.intelligent,@"grade":self.grade,@"tag_id":self.tag_id,@"content":self.content,@"cut_id":self.cut_id}];
    ...
    [YYNet POST:SearchREsultsN paramters:@{@"json":url} success:^(id responseObject) {
        ...
        if ([[dic objectForKey:@"author"] isKindOfClass:[NSArray class]]) {
            ...
            if (self.page == 1) {
                ...
                [self.authordataAry addObjectsFromArray:[dic objectForKey:@"author"]];
            }
            
            NSArray *temp = [dic objectForKey:@"author"];
            if (temp.count < 10) {
                [self.tableView.mj_header endRefreshing];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *temp = [self.authordataAry[indexPath.section] objectForKey:@"author_uuid"];
