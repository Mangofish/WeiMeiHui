...
#define FMLRecordVideoSDKFinishedNotification @"FMLRecordVideoSDKFinishedNotification"

#define FMLRecordViewSDKMinTime 8.0   ///< 最小时间限制
#define FMLRecordViewSDKMaxTime 10.0 ///< 最长时间限制

#define navBarH 40
#define clipFrameViewH 150

@interface VideoCutViewController ()<FMLClipFrameViewDelegate>
...
- (void)setUpNavBar
{
    ...
    [navBar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(SafeAreaHeight);
    }];
...
- (void)setUpClipFrameView:(AVAsset *)asset
{
    ...
    clipFrameView.frame = CGRectMake(0, kHeight-clipFrameViewH, kWidth, clipFrameViewH);
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    CGFloat videoH = kHeight - self.navBar.height - self.clipFrameView.height;
    CGRect videoRect = CGRectMake(0, videoY, kWidth, videoH);
...
- (void)resetDisplayRect
{
    ...
    if (fabs(kHeight - displayRect.size.height) < 5) {
        return;
    }
    ...
    CGFloat diffH = kHeight - navBarH - clipFrameViewH;
    ...
    if (wHRate == 0.75 || displayRect.size.height >= diffH) { // 顶端显示
        ...
        CGFloat diffY = displayRect.origin.y  - navBarH - (kHeight - navBarH - clipFrameViewH - displayRect.size.height) / 2;
...
- (UIView *)iconPlayView
{
    if (!_iconPlayView) {
        ...
        CGFloat playWH = 48;
        CGFloat playX = (kWidth - playWH) / 2;
        CGFloat playY = (kHeight - navBarH - clipFrameViewH - playWH) / 2 + navBarH;
...
- (UIButton *)nextBtn
{
    if (!_nextBtn) {
        ...
        [nextBtn addTarget:self action:@selector(outputAction) forControlEvents:UIControlEventTouchUpInside];
//        操作剪辑后的视频
