...
//    topView.backgroundColor = [UIColor whiteColor];
    ...
    AddressView * topTabbar = [[AddressView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, kHYTopViewHeight)];
    [self addSubview:topTabbar];
    _topTabbar = topTabbar;
    [self addTopBarItemWithString:@"省份"];
    ...
    UIScrollView * contentView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(topTabbar.frame), self.frame.size.width, self.height - kHYTopViewHeight )];
...
- (void)addTopBarItemWithString:(NSString *)name{
    
    UIButton * topBarItem = [UIButton buttonWithType:UIButtonTypeCustom];
    [topBarItem setTitle:name forState:UIControlStateNormal];
    [topBarItem setTitleColor:LightFontColor forState:UIControlStateNormal];
    [topBarItem setTitleColor:FontColor forState:UIControlStateSelected];
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        if(self.cityDataSouce.count == 0){
            ...
            [self setUpAddress:provinceItem.name andCode:provinceItem.code];
            return indexPath;
        }
        ...
        if ([indexPath0 compare:indexPath] != NSOrderedSame && indexPath0) {
            ...
            [self addTopBarItemWithString:@"城市"];
            [self addTableView];
            ...
            [self addTopBarItemWithString:@"区县"];
            [self addTableView];
            [self scrollToNextItem:provinceItem.name];
            return indexPath;
        }
        ...
        [self setUpAddress:item.name andCode:item.code];
    }
    return indexPath;
}
...
- (void)setUpAddress:(NSString *)address andCode:(NSString *)code{
...
//        }
        ...
        if (i==0) {
            self.province = btn.currentTitle;
        }else if (i==1){
            self.city = btn.currentTitle;
        }else{
            self.area = btn.currentTitle;
        }
